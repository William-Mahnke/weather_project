...
JOIN monthly_avg m
   ...
   AND EXTRACT(MONTH FROM s.date) = m.month AND EXTRACT(YEAR FROM s.date) = m.year
WHERE s.mean_temp_f > m.month_avg_temp
GROUP BY c.city_id, c.city_name, m.month, m.year
ORDER BY c.city_name, m.month, m.year;