...
SELECT c.city_name, EXTRACT(YEAR FROM s.date) AS year, EXTRACT(WEEK FROM s.date) AS week, AVG(s.daylight_duration_s) AS avg_daylight
...
ORDER BY c.city_name, year, week;